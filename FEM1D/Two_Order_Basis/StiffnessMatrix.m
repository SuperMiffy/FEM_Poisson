function [A] = StiffnessMatrix(P,Pb,T,Tb,coff_func,deg,der_index_trial,der_index_test)
    ...
    for j = 1:size(Tb,1)%loop with trial basis function
        for k=1:size(Tb,1)%loop with test basis function
           A(Tb(k,i),Tb(j,i)) = A(Tb(k,i),Tb(j,i)) + sum(Gauss_weights.*feval(coff_func,Gauss_nodes).*...
            BasisFunc(vertices,Gauss_nodes,deg,j,der_index_trial).*...
            BasisFunc(vertices,Gauss_nodes,deg,k,der_index_test));
        end
    end
end
end
