...
for i = 1:nofelem
   ...
   x_local = 0;
   for j = 1:size(Tb,1)
      x_local =  x_local + x_FEM1D(Tb(j,i))*BasisFunc(vertices,Gauss_nodes,deg,j,0,0);
   end
   error_L2 = error_L2 + sum(sum(Gauss_weights.*(x_true(Gauss_nodes(:,1),Gauss_nodes(:,2))-x_local).^2));
end
