function [A] = StiffnessMatrix(P,Pb,T,Tb,coff_func,deg,x_der_index_trial,y_der_index_trial,x_der_index_test,y_der_index_test)
...
for i=1:nofelem %loop with element
    ...
    for j = 1:size(Tb,1)%loop with trial basis function
        for k=1:size(Tb,1)%loop with test basis function
           A(Tb(k,i),Tb(j,i)) = A(Tb(k,i),Tb(j,i)) + sum(Gauss_weights.*feval(coff_func,Gauss_nodes(:,1),Gauss_nodes(:,2)).*...
            BasisFunc(vertices,Gauss_nodes,deg,j,x_der_index_trial,y_der_index_trial).*...
            BasisFunc(vertices,Gauss_nodes,deg,k,x_der_index_test,y_der_index_test));
        end
    end
end
end
